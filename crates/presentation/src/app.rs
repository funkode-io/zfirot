//! Root component: gates the board behind a stored Personal Access Token, then
//! keeps it fresh with manual Refresh, a background poll, a "last updated" stamp,
//! and summary counts.
//!
...
use dioxus::prelude::*;
use domain::{format_last_updated, AppErrorKind, BoardSummary, PollInterval, Slice, SliceState};

use crate::components::{
    state_badge_class, state_label, BoardColumn, ErrorBanner, SummaryBar, TokenScreen,
};
...
pub fn App() -> Element {
    ...
    // Bumped after a successful save, a manual Refresh, or a background poll so
    // the token re-resolves and the board reloads.
    let mut reload = use_signal(|| 0u32);
    // Seconds-since-epoch of the most recent completed resolve, formatted for
    // display only once a load has finished.
    let mut last_updated = use_signal(|| None::<u64>);

    let view = use_resource(move || async move {
        let _ = reload(); // subscribe so a saved token / refresh re-resolves
        let resolved = resolve_view().await;
        last_updated.set(Some(now_unix_secs()));
        resolved
    });

    // Background poll: reload on the configured cadence (default ~60s).
    ...
    rsx! {
        ...
        match &*view.read_unchecked() {
            ...
            Some(View::Board(slices)) => rsx! {
                BoardShell {
                    SummaryBar {
                        summary: BoardSummary::from_slices(slices),
                        last_updated: last_updated_label.clone(),
            ...
            // Token present but loading failed for a transient reason.
            Some(View::Error(message)) => rsx! {
                BoardShell {
                    SummaryBar {
                        summary: BoardSummary::from_slices(&[]),
                        last_updated: last_updated_label.clone(),
                        poll_secs: poll.as_secs(),
                        on_refresh: move |_| reload += 1,
                    }
